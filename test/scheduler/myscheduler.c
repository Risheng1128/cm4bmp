...
// 用來儲存每個task的PSP值
uint32_t task_PSP[TASK_NUM] = {TASK1_START_ADDR, TASK2_START_ADDR,
                               TASK3_START_ADDR, TASK4_START_ADDR};
// 儲存每個task_handler的地址
...
void SysTick_Handler(void)
{
    ...
    __asm volatile("MSR PSP, %0" ::"r"(*(task_handler + current_task))
                   :);  // 將TASK1的PSP值複製到PSP裡
...
void SysTick_Timer_Init(void)
{
    ...
    SYST_CSR |=
        (1 << 0) | (1 << 1) | (1 << 2);  // enable ENABLE TICLINT CLKSOURCE
}
...
__attribute__((naked)) void sp_to_psp(void)
{
    ...
    __asm volatile("MSR PSP, %0" ::"r"(*task_handler)
                   :);  // 將TASK1的PSP值複製到PSP裡
