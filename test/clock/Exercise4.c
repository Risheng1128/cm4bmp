 ...
 ******************************************************************************
 * @attention
 *     - Write an application which does PLL configuration to boost the HCLK to
 *maximum capacity
 *       1. APB1: 36MHz
 *       2. APB2: 72MHz
...
void HSI(void)
{
    ...
    RCC_CFGR |=
        (1 << 15);  // PLL entry clock source (HSI used as PREDIV1 entry)
    RCC_CFGR |= (1 << 18) | (1 << 19) |
                (1 << 20);  // PLL multiplication factor (PLL input clock x 9)
    ...
    MYUSART_Init();
    USART3_BRR =
        APB1_CLK_36M / BAUDRATE_38400;  // 重新設定baudrate，覆寫預設結果
}
...
void HSE(void)
{
    ...
    while (!((RCC_CR & 0x20000) >> 17))
        ;  // 等待HSE準備完成
    RCC_CFGR |=
        (1 << 16);  // PLL entry clock source (HSE used as PREDIV1 entry)
    RCC_CFGR &= ~(1 << 17);  // HSE divider for PLL input clock (HSE input to
                             // PLL not divided)
    RCC_CFGR |= (1 << 18) | (1 << 19) |
                (1 << 20);  // PLL multiplication factor (PLL input clock x 9)
    RCC_CR |= (1 << 24);    // PLL enable
