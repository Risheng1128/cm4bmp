 ...
 *          - APB1 : 30MHz
 *          - APB2 : 60MHz
 *     - Use HSE as PLL's input source and repeat the exercise using HSE as
 *input source
...
void HCLK20M(void)
{
    ...
    RCC_CFGR |= (1 << 17);  // HSE divider for PLL input clock (HSE input to PLL
                            // divided by 2)
    RCC_CFGR |= (1 << 18) |
...
void HCLK60M(void)
{
    ...
    RCC_CFGR |=
        (1 << 16);  // PLL entry clock source (HSE used as PREDIV1 entry)
    RCC_CFGR |= (1 << 17);  // HSE divider for PLL input clock (HSE input to PLL
                            // divided by 2)    RCC_CFGR |= (0xd << 18); // PLL
                            // multiplication factor (PLL input clock x 15)
    RCC_CFGR |= (1 << 18) | (1 << 20) |
