 ...
 *     implement the fault exception handlers and cause the fault by following
 *method.
 ...
 *priority whose priority is same or lesser than SVC handler
 */
...
__attribute__((naked)) void Example4(
    void);            // Executing SVC inside the SVC handler
...
{
    uint32_t *SRAM_ptr =
        (uint32_t *) 0x20000501;  // 指到SRAM隨便一個地方 (bit0要1，要對應T bit)
...
{
    void (*func_ptr)(void) =
        (void *) 0x40000000;  // 0x40000000位於peripheral region
    func_ptr();               // 執行函數
}
...
void Example5(
    void)  // Executing SVC instruction inside the interrupt handler whose
           // priority whose priority is same or lesser than SVC handler
{
...
__attribute__((naked)) void HardFault_Handler(void)
{
    ...
    // 根據AAPCS，ARM在函數傳遞引數時，順序為r0, r1, r2,
    // r3，因此r0的值會傳到MSP_ptr
