/**
  ...
  ******************************************************************************
  * @attention
  * 	Exercise: 1. 使用 RTC_WKUP interrupt分析進入中斷後，觀察sp的變化及儲存之暫存為何?
...
int main(void) 
{
    ...
    // __asm volatile("msr PSP, %0"::"r"(psp_value)); // 將psp_valie寫進PSP暫存器
    // __asm volatile("msr CONTROL,%0"::"r"(control_reg)); // 設定CONTROL reg使用PSP暫存器
